...
WINDOW_S_H = 0.1
STEP_S_H = 0.18
# PERFORMANCE (see docs/reproduction_audit.md and Stage 4/05/06): even at a
# matching 0.05s step, a 0.05s (4,800-sample) SE window measured ~3.7s per
# window on real audio here, making the default 5s benchmark take ~6 minutes
# -- too slow for a routine run. Tightened to the same 0.01s window already
# used consistently in notebooks 04-06 for both speed and cross-stage
# comparability. Restore to ~0.0125s/larger for a closer-to-paper-scale
# benchmark once running on adequate hardware/duration (see knob below).
WINDOW_S_SE = 0.01
