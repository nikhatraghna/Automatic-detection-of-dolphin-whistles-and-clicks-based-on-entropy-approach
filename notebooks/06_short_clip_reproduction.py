...
WINDOW_S = 0.1  # NOT SPECIFIED by the paper; representative choice for this plot
...
def plot_spectrogram_plus_entropy(fname, entropy_kind):
    ...
    if entropy_kind == "H":
        step_s = 0.18
        times, vals = sliding_permutation_entropy(data, fs=sr, window_seconds=WINDOW_S, step_seconds=step_s)
        thresh, ylabel = 0.5, "H"
    else:
        step_s = 0.0125
        times, vals = sliding_sample_entropy(data, fs=sr, window_seconds=WINDOW_S, step_seconds=step_s)
