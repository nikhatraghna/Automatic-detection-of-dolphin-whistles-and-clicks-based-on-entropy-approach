...
WINDOW_S = 0.1  # NOT SPECIFIED by paper; single representative choice for this comparison plot
STEP_S_H = 0.18
STEP_S_SE = 0.0125
...
def process_file(fname, filter_fn, filter_kwargs, entropy_kind):
    ...
    if entropy_kind == "H":
        t_u, v_u = sliding_permutation_entropy(data, fs=sr, window_seconds=WINDOW_S, step_seconds=STEP_S_H)
        t_f, v_f = sliding_permutation_entropy(filtered, fs=sr, window_seconds=WINDOW_S, step_seconds=STEP_S_H)
        thresh = 0.5
        ylabel = "H"
    else:
        t_u, v_u = sliding_sample_entropy(data, fs=sr, window_seconds=WINDOW_S, step_seconds=STEP_S_SE)
        t_f, v_f = sliding_sample_entropy(filtered, fs=sr, window_seconds=WINDOW_S, step_seconds=STEP_S_SE)
        thresh = 0.6
        ylabel = "SE"
...
def main():
    ...
    for f in ETS_CLICK_FILES:
        process_file(
            f,
            bandpass_filter,
            {"low_hz": ETS_CLICK_BAND_HZ[0], "high_hz": ETS_CLICK_BAND_HZ[1]},
            "SE",
        )
